...
int main(int argc, char **argv) {
  ...
  mn = std::make_unique<Net>(
      ec, Net::Config().max_msg_size(opt_max_cli_msg->get()));
  ...
  for (const auto &s : opt_replicas->get()) {
    ...
    if (res.size() < 1) throw HotStuffError("format error");
    raw.push_back(res[0]);
  }
  ...
  for (const auto &p : raw) {
    ...
    size_t _;
    replicas.push_back(
        NetAddr(NetAddr(_p.first).ip, htons(stoi(_p.second, &_))));
  }
