...
MsgReqBlock::MsgReqBlock(const std::vector<uint256_t> &blk_hashes) {
  ...
  for (const auto &h : blk_hashes) serialized << h;
}
...
MsgPrepareResp::MsgPrepareResp(const PrepareResp &prep_resp) {
...
promise_t HotStuffBase::async_fetch_blk(const uint256_t &blk_hash,
                                        const PeerId *replica, bool fetch_now) {
  ...
  if (it == blk_fetch_waiting.end()) {
    ...
    it =
        blk_fetch_waiting
            .insert(std::make_pair(blk_hash, BlockFetchContext(blk_hash, this)))
            .first;
  }
  if (replica != nullptr) it->second.add_replica(*replica, fetch_now);
...
promise_t HotStuffBase::async_deliver_blk(const uint256_t &blk_hash,
  ...
  async_fetch_blk(blk_hash, &replica).then([this, replica](block_t blk) {
    ...
    promise::all(pms).then([this, blk](const promise::values_t values) {
      ...
      if (!ret) HOTSTUFF_LOG_WARN("verification failed during async delivery");
    });
  });
...
void HotStuffBase::req_blk_handler(MsgReqBlock &&msg, const Net::conn_t &conn) {
  ...
  std::vector<promise_t> pms;
  for (const auto &h : blk_hashes) pms.push_back(async_fetch_blk(h, nullptr));
...
bool HotStuffBase::conn_handler(const salticidae::ConnPool::conn_t &conn,
                                bool connected) {
  if (connected) {
    if (!pn.enable_tls) return true;
    auto cert = conn->get_peer_cert();
    // SALTICIDAE_LOG_INFO("%s",
    // salticidae::get_hash(cert->get_der()).to_hex().c_str());
...
void HotStuffBase::print_stat() const {
  ...
  for (const auto &replica : peers) {
    ...
    conn->clear_msgstat();
    LOG_INFO("%s: %u(%u), %u(%u), %u", get_hex10(replica).c_str(), ns, nsb, nr,
             nrb, part_fetched_replica[replica]);
    _nsent += ns;
    _nrecv += nr;
...
HotStuffBase::HotStuffBase(uint32_t blk_size, ReplicaID rid,
                           privkey_bt &&priv_key, NetAddr listen_addr,
                           pacemaker_bt pmaker, EventContext ec, size_t nworker,
  ...
  pn.reg_handler(
      salticidae::generic_bind(&HotStuffBase::propose_handler, this, _1, _2));
  pn.reg_handler(
      salticidae::generic_bind(&HotStuffBase::vote_handler, this, _1, _2));
  pn.reg_handler(
      salticidae::generic_bind(&HotStuffBase::req_blk_handler, this, _1, _2));
  pn.reg_handler(
      salticidae::generic_bind(&HotStuffBase::resp_blk_handler, this, _1, _2));
  pn.reg_conn_handler(
      salticidae::generic_bind(&HotStuffBase::conn_handler, this, _1, _2));
  pn.reg_error_handler(
...
void HotStuffBase::start(
  ...
  for (size_t i = 0; i < replicas.size(); i++) {
    ...
    auto peer = pn.enable_tls ? salticidae::PeerId(cert_hash)
                              : salticidae::PeerId(addr);
    ...
    while (q.try_dequeue(e)) {
      ...
      if (cmd_pending_buffer.size() >= blk_size) {
        ...
        pmaker->beat().then([this, cmds = std::move(cmds)](ReplicaID proposer) {
          if (proposer == get_id()) on_propose(cmds, pmaker->get_parents());
        });
        return true;
      }
    }
