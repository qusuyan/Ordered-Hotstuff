...
/** Abstraction for HotStuff protocol state machine (without network
 * implementation). */
class HotStuffCore {
 ...
 public:
  ...
  virtual ~HotStuffCore() { b0->qc_ref = nullptr; }
...
struct Prepare : public Serializable {
  ...
  Prepare(ReplicaID proposer, const block_t &blk, HotStuffCore *hsc)
      : proposer(proposer), blk(blk), hsc(hsc) {}

  void serialize(DataStream &s) const override { s << proposer << *blk; }
...
struct Vote : public Serializable {
  ...
  Vote(ReplicaID voter, const uint256_t &blk_hash, part_cert_bt &&cert,
       HotStuffCore *hsc)
      : voter(voter), blk_hash(blk_hash), cert(std::move(cert)), hsc(hsc) {}
  ...
  promise_t verify(VeriPool &vpool) const {
    assert(hsc != nullptr);
    return cert->verify(hsc->get_config().get_pubkey(voter), vpool)
...
struct Finality : public Serializable {
 ...
 public:
  Finality() = default;
  Finality(ReplicaID rid, int8_t decision, uint32_t cmd_idx,
           uint32_t cmd_height, uint256_t cmd_hash, uint256_t blk_hash)
      : rid(rid),
        decision(decision),
        cmd_idx(cmd_idx),
  ...
  void serialize(DataStream &s) const override {
    s << rid << decision << cmd_idx << cmd_height << cmd_hash;
