...
class PartCert : public Serializable, public Cloneable {
 public:
  uint256_t obj_hash;
  PartCert(){};
  PartCert(uint256_t obj_hash) : obj_hash(obj_hash) {}
...
class QuorumCert : public Serializable, public Cloneable {
 public:
  ...
  QuorumCert(){};
  QuorumCert(uint256_t obj_hash, size_t nreplicas)
...
template <class T>
class PartCertOrder : public PartCert {
 public:
  T order;
  PartCertOrder(){};
  PartCertOrder(uint256_t obj_hash, T order)
      : PartCert(obj_hash), order(order) {}
};

template <class T>
class QuorumCertOrder : public QuorumCert {
 public:
  std::unordered_map<ReplicaID, T> proposed_order;
  QuorumCertOrder(){};
...
class PartCertDummy : public PartCert {
 public:
  PartCertDummy() {}
  PartCertDummy(const uint256_t &obj_hash) : PartCert(obj_hash) {}

  void serialize(DataStream &s) const override { s << (uint32_t)2 << obj_hash; }
  ...
  PartCert *clone() override { return new PartCertDummy(obj_hash); }
...
class QuorumCertDummy : public QuorumCert {
 public:
  QuorumCertDummy(){};
  QuorumCertDummy(const ReplicaConfig &config, const uint256_t &obj_hash);

  void serialize(DataStream &s) const override {
    s << (uint32_t)3 << obj_hash << rids;
  }
  ...
  QuorumCert *clone() override { return new QuorumCertDummy(*this); }
...
class PartCertOrderDummy : public PartCertOrder<std::vector<uint32_t>> {
 public:
  PartCertOrderDummy() {}
  PartCertOrderDummy(const uint256_t &obj_hash,
                     const std::vector<uint32_t> order)
      : PartCertOrder(obj_hash, order) {}

  void serialize(DataStream &s) const override {
    s << (uint32_t)0 << obj_hash << order.size();
    for (const auto &id : order) s << id;
  }
  ...
  void unserialize(DataStream &s) override {
    ...
    s >> size;
    order.resize(size);
    for (auto &id : order) s >> id;
  }

  PartCertOrder<std::vector<uint32_t>> *clone() override {
    return new PartCertOrderDummy(obj_hash, order);
  }
...
class QuorumCertOrderDummy : public QuorumCertOrder<std::vector<uint32_t>> {
 public:
  ...
  void serialize(DataStream &s) const override {
    ...
    for (size_t i = 0; i < rids.size(); i++)
      if (rids.get(i)) {
        auto order = proposed_order.at(i);
        s << order.size();
        for (const auto &id : order) s << id;
      }
  }
  ...
  void unserialize(DataStream &s) override {
    ...
    for (size_t i = 0; i < rids.size(); i++)
      if (rids.get(i)) {
        ...
        order.resize(size);
        for (auto &id : order) s >> id;
        proposed_order.insert(std::make_pair(i, std::move(order)));
      }
  }

  QuorumCertOrderDummy *clone() override {
    return new QuorumCertOrderDummy(*this);
  }
  ...
  void add_part(ReplicaID rid, const PartCert &pc) override {
    ...
    proposed_order.insert(
        std::make_pair(rid, static_cast<const PartCertOrderDummy &>(pc).order));
    rids.set(rid);
  }
...
class Secp256k1Context {
 ...
 public:
  Secp256k1Context(bool sign = false)
      : ctx(secp256k1_context_create(sign ? SECP256K1_CONTEXT_SIGN
...
class PubKeySecp256k1 : public PubKey {
 ...
 public:
  ...
  void serialize(DataStream &s) const override {
    ...
    size_t olen = _olen;
    (void)secp256k1_ec_pubkey_serialize(ctx->ctx, (unsigned char *)output,
  ...
  void unserialize(DataStream &s) override {
    ...
    try {
      if (!secp256k1_ec_pubkey_parse(ctx->ctx, &data, s.get_data_inplace(_olen),
                                     _olen))
        throw _exc;
  ...
  PubKeySecp256k1 *clone() override { return new PubKeySecp256k1(*this); }
};
...
class PrivKeySecp256k1 : public PrivKey {
 ...
 public:
  PrivKeySecp256k1(const secp256k1_context_t &ctx = secp256k1_default_sign_ctx)
      : PrivKey(), ctx(ctx) {}
...
PubKeySecp256k1::PubKeySecp256k1(const PrivKeySecp256k1 &priv_key,
...
class SigSecp256k1 : public Serializable {
 ...
 public:
  ...
  SigSecp256k1(const uint256_t &digest, const PrivKeySecp256k1 &priv_key,
  ...
  void serialize(DataStream &s) const override {
    ...
    (void)secp256k1_ecdsa_signature_serialize_compact(
        ctx->ctx, (unsigned char *)output, &data);
  ...
  void unserialize(DataStream &s) override {
    ...
    try {
      if (!secp256k1_ecdsa_signature_parse_compact(ctx->ctx, &data,
                                                   s.get_data_inplace(64)))
        throw _exc;
    } catch (std::ios_base::failure &) {
      throw _exc;
    }
  ...
  void sign(const bytearray_t &msg, const PrivKeySecp256k1 &priv_key) {
    ...
    if (!secp256k1_ecdsa_sign(ctx->ctx, &data, (unsigned char *)&*msg.begin(),
  ...
  bool verify(const bytearray_t &msg, const PubKeySecp256k1 &pub_key,
    ...
    check_msg_length(msg);
    return secp256k1_ecdsa_verify(_ctx->ctx, &data,
...
class Secp256k1VeriTask : public VeriTask {
 ...
 public:
  Secp256k1VeriTask(const uint256_t &msg, const PubKeySecp256k1 &pubkey,
...
class PartCertSecp256k1 : public SigSecp256k1, public PartCert {
 public:
  ...
  PartCertSecp256k1(const PrivKeySecp256k1 &priv_key, const uint256_t &obj_hash)
      : SigSecp256k1(obj_hash, priv_key), PartCert(obj_hash) {}
  ...
  promise_t verify(const PubKey &pub_key, VeriPool &vpool) override {
    return vpool.verify(new Secp256k1VeriTask(
        obj_hash, static_cast<const PubKeySecp256k1 &>(pub_key),
        static_cast<const SigSecp256k1 &>(*this)));
  }

  PartCertSecp256k1 *clone() override { return new PartCertSecp256k1(*this); }
 ...
 public:
  ...
  void add_part(ReplicaID rid, const PartCert &pc) override {
    if (pc.get_obj_hash() != obj_hash)
      throw std::invalid_argument("PartCert does match the block hash");
    sigs.insert(
        std::make_pair(rid, static_cast<const PartCertSecp256k1 &>(pc)));
    rids.set(rid);
  }
