...
class PMHighTail : public virtual PaceMaker {
  ...
  bool check_ancestry(const block_t &_a, const block_t &_b) {
    block_t b;
    for (b = _b; b->get_height() > _a->get_height(); b = b->get_parents()[0])
      ;
    return b == _a;
  }
  ...
  void reg_hqc_update() {
    hsc->async_hqc_update().then([this](const block_t &hqc) {
      ...
      for (const auto &tail : hsc->get_tails())
        if (check_ancestry(hqc, tail) &&
            tail->get_height() > hqc_tail->get_height())
          hqc_tail = tail;
      reg_hqc_update();
    });
  }
  ...
  void reg_receive_proposal() {
    hsc->async_wait_receive_proposal().then([this](const Proposal &prop) {
      ...
      if (check_ancestry(hqc, blk) &&
          blk->get_height() > hqc_tail->get_height())
        hqc_tail = blk;
...
class PMWaitQC : public virtual PaceMaker {
 ...
 protected:
  ...
  void update_last_proposed() {
    ...
    (pm_wait_propose = hsc->async_wait_proposal())
        .then([this](const Proposal &prop) {
 ...
 public:
  ...
  ReplicaID get_proposer() override { return hsc->get_id(); }
  ...
  promise_t beat_resp(ReplicaID last_proposer) override {
    return promise_t(
        [last_proposer](promise_t &pm) { pm.resolve(last_proposer); });
  }
};
...
class PaceMakerDummyFixed : public PaceMakerDummy {
 ...
 public:
  PaceMakerDummyFixed(ReplicaID proposer, int32_t parent_limit)
      : PaceMakerDummy(parent_limit), proposer(proposer) {}

  ReplicaID get_proposer() override { return proposer; }

  promise_t beat_resp(ReplicaID) override {
    return promise_t([this](promise_t &pm) { pm.resolve(proposer); });
  }
};
...
class PMRoundRobinProposer : virtual public PaceMaker {
  ...
  void rotate() {
    ...
    // start timer
    timer =
        TimerEvent(ec, salticidae::generic_bind(
                           &PMRoundRobinProposer::on_exp_timeout, this, _1));
  ...
  void stop_rotate() {
    ...
    if (proposer == hsc->get_id()) {
      ...
      hs->get_tcall().async_call([this, hs](salticidae::ThreadCall::Handle &) {
        ...
        for (auto &p : pending) cmds.push_back(p.first);
 ...
 protected:
  void on_consensus(const block_t &blk) override {
    ...
    if (prop_blk[proposer] == blk) stop_rotate();
  }
 ...
 public:
  PMRoundRobinProposer(const EventContext &ec, double base_timeout,
                       double prop_delay)
  ...
  promise_t beat() override {
    if (!rotating && proposer == hsc->get_id()) {
      ...
      return promise_t(
          [proposer = proposer](promise_t &pm) { pm.resolve(proposer); });
  }
...
struct PaceMakerRR : public PMHighTail, public PMRoundRobinProposer {
  PaceMakerRR(EventContext ec, int32_t parent_limit, double base_timeout = 1,
              double prop_delay = 1)
